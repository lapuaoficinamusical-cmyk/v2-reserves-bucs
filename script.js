...
function updateStatusSummary() {
  const lastCart = cart[cart.length - 1];
  const hasSelection = startCell !== null && endCell !== null;

  statusBuc.textContent = selectedBuc ? `Buc ${selectedBuc}` : (lastCart ? `Buc ${lastCart.buc}` : "Pendent");
  statusDate.textContent = selectedDate ? formatDateLabel(selectedDate) : (lastCart ? formatDateLabel(lastCart.date) : "Selecciona un dia");

  if (hasSelection) {
    statusTime.textContent = `${TIMES[startCell]} – ${TIMES[endCell + 1]}`;
  } else if (lastCart) {
    statusTime.textContent = `${TIMES[lastCart.startCell]} – ${TIMES[lastCart.endCell + 1]}`;
  } else {
